...
def predict_next_location(predict_locations, speed):
    """
    پیش‌بینی موقعیت بعدی بر اساس جهت مسیر اخیر و سرعت داده‌شده.
    اگر مسیر کمتر از سه نقطه داشته باشد، آخرین موقعیت برگردانده می‌شود.
    """
    # اگر مسیر کمتر از 3 نقطه داشت، آخرین موقعیت را برگردان
    if len(predict_locations) < 3:
        return predict_locations[-1]

    # آخرین سه نقطه
    pts = np.array(predict_locations[-3:])
    t = np.array([0, 1, 2])
    
    # برازش درجه دو برای X و Y
    coef_x = np.polyfit(t, pts[:, 0], 2)
    coef_y = np.polyfit(t, pts[:, 1], 2)
    
    # مشتق (سرعت لحظه‌ای) در آخرین نقطه
    der_x = np.polyder(coef_x)
    der_y = np.polyder(coef_y)
    vx = np.polyval(der_x, 2)
    vy = np.polyval(der_y, 2)
    ...
    if norm == 0:
        ...
        norm = np.linalg.norm(direction)
    if norm == 0:
        return predict_locations[-1]  # اگر مسیر کاملاً ثابت بود
    
    unit_dir = direction / norm
    
    # موقعیت بعدی
    last_loc = pts[-1]
    next_loc = last_loc + unit_dir * speed
    next_loc_int = tuple(map(int, next_loc.tolist()))
...
def test(args, wandb_log):
    ...
    for i in range(97):
        try:
            ...
            last_step = predict_next_location(predict_locations=predict_locations,speed= 200*5)
            img1=crop_center(map,last_step, 600*5, 600*5)
            ...
            last_location = (
                int(last_step[0] + x_center*5 - 1500),
                int(last_step[1] + y_center*5 - 1500)
            )
